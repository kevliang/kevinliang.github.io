var taskInput = document.getElementById("autocomplete"),
    ...
    noflying = document.getElementById("noFlying"),
    incompleteTaskHolder = document.getElementById("incomplete-tasks");
$("#incomplete-tasks").disableSelection(), $("#autocomplete").keydown(function(e) {
...
function addMarker(e) {
    ...
    var a = {
        ...
        startY: t.y,
        fill: "#f47825",
        flying: 1 == $("#journeytype [name='journeytype']:checked").val(),
        current: !0,
...
function updateEmissionData() {
    $("#distancebetween [data-distance]").text(map.distance), map.calculateEmission(), $("#totalEmission[data-emission]").text(parseInt(map.emission)), document.querySelector("#totalCities").innerHTML = document.querySelector("#incomplete-tasks").childElementCount, $("#emission-data").show()
}

function updateLabelText() {
    $("#journeytype").val("1"), map.markers.length <= 0 ? ($("#sort-it [for='autocomplete']").text("I'm starting my CaRMS Tour from"), $("#journeytype").hide()) : ($("#sort-it [for='autocomplete']").text("I then travelled to"), $("#journeytype").show()), map.markers.length > 1 ? $("#calculate").show() : ($("#calculate").hide(), $("#emission-data").hide()), $("#emission-data").is(":visible") && (updateEmissionData(), runInfographic(), drawCharts())
}

function animateThankYouMessage() {
    $("#thankyou").show(), $("html, body").stop().animate({
        scrollTop: $("#thankyou").position().top
    }, 500)
}

function updateShamePlane() {
    document.getElementById("opt1").checked = !1, document.getElementById("opt2").checked = !1, document.getElementById("opt4").checked = !1, document.getElementById("opt5").checked = !1, document.getElementById("opt7").checked = !1, document.getElementById("opt1-bar").classList.remove("on"), document.getElementById("opt2-bar").classList.remove("on"), document.getElementById("opt4-bar").classList.remove("on"), document.getElementById("opt5-bar").classList.remove("on"), document.getElementById("opt7-bar").classList.remove("on")
}


noflying.addEventListener("click", function() {
    updateEmissionData(), updateFormData(), $("html, body").stop().animate({
        scrollTop: $("#emission-data").position().top
    }, 500), runInfographic(), drawCharts(), updateShamePlane(), document.getElementById("schooloforigin").focus()
});
