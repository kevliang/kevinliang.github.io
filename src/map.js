map = new Vue({
    ...
    filters: {
        numberWithCommas: r => r && r.toString ? r.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : r
    },
    mounted() {
        var r = d3.geoAlbers().parallels([50, 70]).rotate([90, 0, 0]).scale([1500]);
        this.projection = r, fetch("data/canada.json").then(r => r.json()).then(e => {
            var t = d3.geoPath().projection(r);
            d3.select(this.$refs.provinces).selectAll("path").data(e.features).enter().append("path").attr("d", t)
        }), fetch("data/canadianairport.json").then(r => r.json()).then(e => {
    ...
    methods: {
        ...
        markerStop() {
            console.log("stop!"), document.removeEventListener("mousemove", this.markerDrag), document.removeEventListener("mouseup", this.markerStop), this.$refs.map.removeEventListener("mouseleave", this.markerLeave), this.currentMarker.current = !1, this.currentMarker = null, this.markerDistance()
        },
        markerConnect() {
        ...
        calcDistance(r, e, t, a, i) {
            ...
            return l = 60 * (l = 180 * (l = Math.acos(l)) / Math.PI) * 1.1515, "K" == i && (l *= 1.609344), "N" == i && (l *= .8684), l *= 1.60934
        },
        markerDistance() {
        ...
        airplaneAnimate() {
            var r = this.markerConnect(),
                ...
                a = Math.min(.2 * t, .3);
            this.airplaneFade && this.airplaneFade.isActive() ? (e.pause(), this.airplaneFade.eventCallback("onComplete", function() {
                e.play()
            })) : this.airplaneTween && this.airplaneTween.kill();
            var i = MorphSVGPlugin.pathDataToBezier(r.join(" "));
            e.to(this.$refs.airplane, t, {
                bezier: {
                    values: i,
                    curviness: 1,
                    autoRotate: -90,
                ...
                reversed: !0,
                ease: Linear.easeNone
            }, 0), e.fromTo(this.$refs.airplane, a, {
                opacity: 0
            }, {
                opacity: 1,
                delay: a / 2,
                ease: "Linear.easeNone"
            }, 0), e.to(this.$refs.airplane, a, {
                opacity: 0,
                ease: "Linear.easeNone"
            }, "-=" + a), this.airplaneTween = e
        },
        calculateEmission() {
            this.emission = 0;
            for (var r = 0, e = 0, t = null, a = null, i = null, s = null, n = 0; n < this.markers.length; n++) this.markers[n].flying && (t ? (i = this.markers[n], s = this.projection.invert([i.x, i.y]).reverse()) : (t = this.markers[n], a = this.projection.invert([t.x, t.y]).reverse())), t && i && (r = Math.round(this.calcDistance(a[0], a[1], s[0], s[1])), e += r *= r < 463 ? .25493 : r > 463 && r < 3700 ? .15573 : .14981, t = i, a = s, i = null, s = null);
            this.emission = e
        }
    }
});